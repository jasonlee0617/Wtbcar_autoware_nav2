...
public:
    ...
    bool cachePointCloud(const sensor_msgs::msg::PointCloud2::SharedPtr& laserCloudMsg)
    {
        ...
        {
            // LiDAR driver records time as a negative offset from scan END
            // (e.g. LSLiDAR C16).  Header timestamp marks the end of the
            // scan and per-point timestamps are ≤ 0.
            timeScanCur = cloudStampSec + pcl_first_time;  // true scan start
            timeScanEnd = cloudStampSec;                   // true scan end
        }
        else
        {
            // Velodyne convention: header = scan START, points.time ≥ 0.
            timeScanCur = cloudStampSec;
            timeScanEnd = cloudStampSec + pcl_last_time;
        }
        ...
        if (ringFlag == 0)
        {
            ...
            for (int i = 0; i < (int)currentCloudMsg.fields.size(); ++i)
            {
                if (currentCloudMsg.fields[i].name == "ring" ||
                    currentCloudMsg.fields[i].name == "channel")
                {
                    ringFlag = 1;
                    break;
                }
